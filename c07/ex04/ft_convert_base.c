...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/07/18 11:24:48 by cstephan          #+#    #+#             */
/*   Updated: 2020/07/22 10:43:57 by cstephan         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_putnbr_base(char *res, int nbr, char *base, int *last_n)
{
	...
	if (nbr <= -2147483648)
	{
		res[0] = '-';
		(*last_n)++;
		ft_putnbr_base(res, (-(long)nbr) / b, base, last_n);
		ft_putnbr_base(res, (-(long)nbr) % b, base, last_n);
	}
	else if (nbr < 0)
	{
...
char		*ft_convert_base(char *nbr, char *base_from, char *base_to)
{
	...
	if (!(res = malloc(sizeof(char) * (count_digits(n, base_to) + 1))))
		return (0);
	last_n = 0;
